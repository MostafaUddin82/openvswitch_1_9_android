...
struct rule_dpif {
	...
	uint32_t current_index;			 /*Mostafa */
	uint64_t arrival_time[FIRST_N];  /* Mostafa */
...
flush(struct ofproto *ofproto_)
{
    ...
    HMAP_FOR_EACH_SAFE (facet, next_facet, hmap_node, &ofproto->facets) {
        ...
        LIST_FOR_EACH (subfacet, list_node, &facet->subfacets) {
			...
			/* mostafa */
			subfacet->current_index = 0; 
			memset(subfacet->arrival_time, 0, FIRST_N * 8);
			/*
			for (i=0; i<FIRST_N; i++) {
				subfacet->arrival_time[i]=0;
			}
			*/
			/* mostafa */
        }
                      ...
                      const struct dpif_flow_stats *stats)
{
	...
	int i,j,k;    // mostafa
	uint64_t tmp_arrival_time[FIRST_N];  /* Mostafa */

	VLOG_DBG("update_subfacet_stats \n");
	...
	for(k=0,i=0,j=0;(i<stats->current_index||j<facet->current_index)&&(k<FIRST_N);k++){
		if(stats->arrival_time[i]>facet->arrival_time[j]){
			tmp_arrival_time[k] = stats->arrival_time[i];
			i++;
		}
		else {
			tmp_arrival_time[k] = facet->arrival_time[j];
			j++;
		}
	}
	facet->current_index = k;
	for(k=0;k<facet->current_index;k++){
		facet->arrival_time[k] = tmp_arrival_time[k];
	}
	
	subfacet->current_index = stats->current_index;
	//memcpy(subfacet->arrival_time,stats->arrival_time, 8 * FIRST_N);
	
	for (i=0; i<stats->current_index; i++) {
		subfacet->arrival_time[i]=stats->arrival_time[i];
	}
...
facet_flush_stats(struct facet *facet)
{
	...
	for(k=0,i=0,j=0;(i<facet->current_index||j<facet->rule->current_index)&&(k<FIRST_N);k++){
		if(facet->arrival_time[i]>facet->rule->arrival_time[j]){
			tmp_arrival_time[k] = facet->arrival_time[i];
			i++;
		}
		else {
			tmp_arrival_time[k] = facet->rule->arrival_time[j];
			j++;
		}
	}
	facet->rule->current_index = k;
	for(k=0;k<facet->rule->current_index;k++){
		facet->rule->arrival_time[k] = tmp_arrival_time[k];
	}
...
facet_reset_counters(struct facet *facet)
{
	...
	facet->current_index=0; //mostafa
	memset(facet->arrival_time, 0, FIRST_N * 8); //mostafa
}
...
facet_push_stats(struct facet *facet)
{
	...
	/*  Mostafa */
	stats.current_index = facet->current_index;
	for (i=0; i<facet->current_index; i++) {
		stats.arrival_time[i] = facet->arrival_time[i];
	}
	/*  Mostafa */
...
rule_credit_stats(struct rule_dpif *rule, const struct dpif_flow_stats *stats)
{
	...
	for(k=0,i=0,j=0;(i<stats->current_index||j<rule->current_index)&&(k<FIRST_N);k++){
		if(stats->arrival_time[i]>rule->arrival_time[j]){
			tmp_arrival_time[k] = stats->arrival_time[i];
			i++;
		}
		else {
			tmp_arrival_time[k] = rule->arrival_time[j];
			j++;
		}
	}
	rule->current_index = k;
	for(k=0;k<rule->current_index;k++){
		rule->arrival_time[k] = tmp_arrival_time[k];
	}
                ...
                ovs_be16 initial_tci, long long int now)
{
	...
	subfacet->current_index = 0;   //mostafa
	memset(subfacet->arrival_time, 0, FIRST_N * 8); //mostafa
...
{
	int i,j,k;
	uint64_t tmp_arrival_time[FIRST_N];
	
	VLOG_DBG("subfacet_update_stats \n");
...
rule_construct(struct rule *rule_)
{
	...
	rule->current_index=0; //mostafa
	memset(rule->arrival_time, 0, FIRST_N * 8); //mostafa
    ...
    *bytes = rule->byte_count;
    ...
    LIST_FOR_EACH (facet, list_node, &rule->facets) {
        ...
        *bytes += facet->byte_count;
		...
		for(k=0,i=0,j=0;(i<facet->current_index||j<rule->current_index)&&(k<FIRST_N);k++){
			if(facet->arrival_time[i]>rule->arrival_time[j]){
				tmp_arrival_time[k] = facet->arrival_time[i];
				i++;
			}
			else {
