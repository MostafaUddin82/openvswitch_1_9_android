...
void ovs_flow_used(struct sw_flow *flow, struct sk_buff *skb)
{
    ...
    ktime_get_ts(&cur_ts);
    cur_us = (u64)cur_ts.tv_sec * USEC_PER_SEC +
	...
	/* mostafa */
	if(flow->current_index<=(FIRST_N-1)){
		flow->arrival_time[flow->current_index] = cur_ts;
		flow->current_index = (flow->current_index+1);
	}
	else if(flow->current_index>(FIRST_N-1)){
		for(i=1;i<=FIRST_N-1;i++){
			flow->arrival_time[i-1] = flow->arrival_time[i];
		}
		flow->arrival_time[i-1] = cur_ts;
	}
	/* mostafa */
