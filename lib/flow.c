...
parse_ipv6(struct ofpbuf *packet, struct flow *flow)
{
    ...
    //flow->ipv6_src = nh->ip6_src;  Previous statement
    //flow->ipv6_dst = nh->ip6_dst;  Previous statement
	
	
	for(i=0;i<8;i++){
		flow->ipv6_src.s6_addr16[i] = nh->ip6_src.s6_addr16[i];
		flow->ipv6_dst.s6_addr16[i] = nh->ip6_dst.s6_addr16[i];
	}
...
parse_icmpv6(struct ofpbuf *b, struct flow *flow)
{
    ...
    if (icmp->icmp6_code == 0 &&
		...
		/* flow->nd_target = *nd_target;  Previous statement*/
		for(i=0;i<8;i++){
				flow->nd_target.s6_addr16[i] = nd_target->s6_addr16[i];
		}
