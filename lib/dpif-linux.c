...
dpif_linux_flow_dump_start(const struct dpif *dpif_, void **statep)
{
	...
	VLOG_DBG("dpif_linux_flow_dump_start  \n");
    *statep = state = xmalloc(sizeof *state);
                          ...
                          const struct dpif_flow_stats **stats)
{
    ...
    struct ofpbuf buf;
    int error;
	VLOG_DBG("dpif_linux_flow_dump_next \n");
    do {
...
dpif_linux_flow_dump_done(const struct dpif *dpif OVS_UNUSED, void *state_)
{
    struct dpif_linux_flow_state *state = state_;
    int error = nl_dump_done(&state->dump);
    VLOG_DBG("dpif_linux_flow_dump_done \n");
	ofpbuf_delete(state->buf);
    free(state);
    return error;
}
                          ...
                          struct dpif_flow_stats *stats)
{
    ...
    if (flow->stats) {
		...
		/*mostafa */
		stats->current_index = flow->stats->current_index;
		for (i=0;i<FIRST_N;i++) {
            stats->arrival_time[i] = get_unaligned_u64(&flow->stats->arrival_time[i]);
        }
		/* mostafa */
		...
		memset(stats->arrival_time, 0, 8*FIRST_N);
		/* mostafa */
    }
